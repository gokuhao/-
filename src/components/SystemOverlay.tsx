        ...
        <main className="workbench-content">
          ...
          {activeView === "settings" && settings && (
            <div className="workbench-settings-page">
              ...
              <section><h2>边缘互动</h2><label className="workbench-select-row"><span><strong>互动强度</strong><small>控制宠物贴边后的主动反馈</small></span><select value={settings.edgeInteractionMode} disabled={busy} onChange={(event) => void saveSettings({ ...settings, edgeInteractionMode: event.target.value as StepBeastEdgeInteractionMode })}><option value="quiet">安静 · 只裁边</option><option value="standard">标准 · 探出观察</option><option value="lively">活泼 · 挥手问候</option></select></label></section>
              <section><h2>显示大小</h2><label className="workbench-select-row"><span><strong>桌面宠物大小</strong><small>只影响收起后常驻桌面的宠物</small></span><select value={settings.petScale} disabled={busy} onChange={(event) => void saveSettings({ ...settings, petScale: Number(event.target.value) as 0.75 | 1 | 1.25 })}><option value={0.75}>小号 · 75%</option><option value={1}>标准 · 100%</option><option value={1.25}>大号 · 125%</option></select></label><label className="workbench-select-row"><span><strong>操作窗口大小</strong><small>影响今日面板和完整工作台</small></span><select value={settings.panelScale} disabled={busy} onChange={(event) => void saveSettings({ ...settings, panelScale: Number(event.target.value) as 0.9 | 1 | 1.1 })}><option value={0.9}>紧凑 · 90%</option><option value={1}>标准 · 100%</option><option value={1.1}>放大 · 110%</option></select></label></section>
