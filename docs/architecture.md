...
- 以 `type: project_index` 的项目总览为唯一入口，只沿“当前项目 / 支撑系统 / 暂停项目”Wikilink 读取一层项目主页。
- 同时兼容 Vault 根路径和项目目录相对 Wikilink；只在项目主页明确链接“当前进度”时再向下读取一层。
- 状态优先使用 `project_status`，目标、阶段和下一步只从明确章节提取，不用 AI 猜测缺失事实。
- 任务候选默认不选择，一次最多确认 5 个；暂停项目和支撑系统不能生成执行任务。
- 项目 upsert、所选任务创建、稳定候选去重和审计事件在一个 SQLite 事务中完成。
- 项目提案只读取 Obsidian；当前版本不会反向修改 Markdown。
