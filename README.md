...
- 安装版提供正式小猫桌面图标、开始菜单入口和系统托盘；托盘支持显示、隐藏、开机启动、重启与彻底退出。
- 使用单实例保护；重复双击桌面图标不会生成第二只宠物，而是恢复已经运行的步步兽。
- 关闭窗口只隐藏到托盘并保留后台计时；工作台或托盘的“彻底退出”会先清理提醒与 SQLite，再结束全部进程。
- 设置中可切换宠物 75%/100%/125% 和操作窗口 90%/100%/110% 三档大小。
- 设置中可切换安静、标准、活泼三档边缘互动；默认标准，旧设置会自动补全且无需迁移数据库。
...
- 已配置 `electron-builder`，可生成带正式图标的 Windows x64 NSIS 安装程序和桌面/开始菜单快捷方式。
- 尊重系统的“减少动态效果”设置。
